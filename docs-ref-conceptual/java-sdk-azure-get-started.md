...
이 가이드에서는 Azure 서비스 사용자를 사용하여 개발 환경을 설정하고, Java용 Azure 라이브러리를 사용하여 Azure 구독에서 리소스를 만들고 사용하는 샘플 코드를 실행하는 과정을 안내합니다.
...
[Azure CLI 2.0을 사용하여 서비스 사용자를 만들고](/cli/azure/create-an-azure-service-principal-azure-cli) 출력을 캡처합니다. 암호 인수에 `MY_SECURE_PASSWORD` 대신 [보안 암호](https://docs.microsoft.com/azure/active-directory/active-directory-passwords-policy)를 제공해야 합니다.
...
코드에서 읽을 수 있는 시스템의 안전한 위치에 이 파일을 저장합니다. 셸에서 인증 파일의 전체 경로가 포함된 `AZURE_AUTH_LOCATION` 환경 변수를 설정합니다.    
